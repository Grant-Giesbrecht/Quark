...
vector<string> generate_LUT(map<string, operation> ops, vector<control_line> controls){
	...
	for (size_t inst_idx = 0 ; inst_idx < 128 ; inst_idx++){ // For each instruction... (Number of instructions: 2^7)

		for (size_t phs_idx = 0 ; phs_idx < 16 ; phs_idx++){ // For each phase...

			for (size_t ch_idx = 0 ; ch_idx < 16 ; ch_idx++){ // For each channel/word

				for (size_t pin_idx = 0 ; pin_idx < 8 ; pin_idx++){ // For each channel
